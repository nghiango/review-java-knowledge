...
public class SafeFundsTransferController {

    private final Map<String, BigDecimal> balances = new ConcurrentHashMap<>();

    public SafeFundsTransferController() {
    ...
    public ResponseEntity<Map<String, Object>> transfer(
        ...
        balances.compute(
                request.recipient(),
                (k, v) -> (v == null ? BigDecimal.ZERO : v).add(request.amount()));
    ...
    public record TransferRequest(
            @NotBlank String recipient, @NotNull @DecimalMin(value = "0.01") BigDecimal amount) {}
}
