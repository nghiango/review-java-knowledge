...
/**
 * Architectural fitness function tests using ArchUnit. Automatically verifies Hexagonal boundaries,
 * modular monolith slices, and code conventions.
 */
class ArchitectureFitnessTest {
    ...
    @Test
    @DisplayName(
            "Clean Architecture: Domain model must not depend on infrastructure, application, Spring or JPA")
    void domainMustNotDependOnInfrastructureOrFrameworks() {
        noClasses()
                ...
                .resideInAnyPackage(
                        ...
                        "jakarta.persistence..")
                .because(
                        "Pure domain models must be free of framework and infrastructure dependencies")
                .check(importedClasses);
    }

    @Test
    @DisplayName(
            "Clean Architecture: Application service must not depend on infrastructure adapters")
    void applicationServiceMustNotDependOnInfrastructureAdapters() {
        noClasses()
                .that()
                .resideInAPackage("..cleanarchitecture.application..")
                .should()
                .dependOnClassesThat()
                .resideInAPackage("..cleanarchitecture.infrastructure.adapter..")
                .because(
                        "Application layer should only depend on domain ports, not concrete infrastructure adapters")
                .check(importedClasses);
    }
    ...
    void modulesMustNotAccessOtherModulesInternalPackages() {
        noClasses()
                .that()
                .resideInAPackage("..modularmonolith.billing..")
                .should()
                .dependOnClassesThat()
                .resideInAPackage("..modularmonolith.inventory.internal..")
                .because(
                        "Billing module must interact with Inventory only via its public InventoryApi")
                .check(importedClasses);
    }
    ...
    void noFieldInjection() {
        fields().that()
                ...
                .resideInAPackage("lab.architecture..")
                .should()
