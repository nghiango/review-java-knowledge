...
public final class Q27WebhookVsPollingAsyncPatternExample {
    ...
    // Client receives HTTP 202 with Location header pointing to task status endpoint and
    // Retry-After header.
    ...
    // Server asynchronously sends HTTP POST to client-provided callback URL with HMAC signature
    // header:
    public static class WebhookDispatcher {
        public static String buildHmacHeader(String payload, String secret) {
            // Generates HMAC-SHA256 signature (e.g. X-Hub-Signature-256) so client can verify
            // authenticity
    ...
    public static void main(String[] args) {
        ...
        String location =
                response.getHeaders().getLocation().toString(); // "/api/v1/tasks/task-9988"

        String signature =
                WebhookDispatcher.buildHmacHeader("{\"event\":\"task.completed\"}", "secret-key");
