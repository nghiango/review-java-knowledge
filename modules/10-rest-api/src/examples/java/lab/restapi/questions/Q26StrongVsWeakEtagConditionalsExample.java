...
public final class Q26StrongVsWeakEtagConditionalsExample {
    ...
    // Strong ETag ("\"hash\""): Byte-for-byte exact representation match. Required for HTTP Range
    // requests.
    // Weak ETag (W/"\"hash\""): Semantic equivalence (e.g. gzip compressed vs uncompressed, or
    // timestamp-based).
    public static class EtagValidator {
        ...
        public ResponseEntity<String> updateResource(String ifMatchHeader, String newContent) {
            ...
            // another user modified the resource concurrently -> return HTTP 412 Precondition
            // Failed!
            ...
            return ResponseEntity.ok().header(HttpHeaders.ETAG, currentStrongEtag).body(newContent);
        }

        public String getEtag() {
            return currentStrongEtag;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        int status1 =
                staleAttempt
                        .getStatusCode()
                        .value(); // 412 (Precondition Failed, prevents lost update!)

        // Client B with fresh ETag succeeds:
        ResponseEntity<String> validAttempt =
                validator.updateResource("\"v1-exact-bytes\"", "new data");
