...
public final class Q25CteMaterializedOptimizationFenceExample {
    ...
    // In PostgreSQL 12+, Common Table Expressions (CTEs) are inlined automatically (NOT MATERIALIZED)
    // allowing the query planner to push outer WHERE predicates down into the CTE query.
    //
    // Specifying WITH ... AS MATERIALIZED:
    // Forces PostgreSQL to evaluate the CTE in isolation once and write results to a temporary buffer,
    // acting as an intentional optimization fence to prevent expensive re-evaluations or control join order.
    ...
    public static void main(String[] args) {
        ...
        boolean pushesPredicate = !inlined.contains("MATERIALIZED"); // true (predicate pushed down into scan)
