...
public final class Q27ZeroDowntimeColumnMigrationExample {
    ...
    // Expand and Contract 5-phase migration sequence for renaming or transforming a database
    // column:
    // Phase 1 (Expand): Add new column as nullable (ALTER TABLE users ADD COLUMN full_name
    // VARCHAR(255);)
    // Phase 2 (Dual Write): Deploy application version N+1 that writes to both old and new columns,
    // reading from old.
    // Phase 3 (Backfill): Run background worker in batches to populate historical rows from old to
    // new column.
    // Phase 4 (Switch Read): Deploy application version N+2 that reads from new column and writes
    // to both.
