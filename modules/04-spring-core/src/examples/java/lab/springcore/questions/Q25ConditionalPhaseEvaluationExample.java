...
public final class Q25ConditionalPhaseEvaluationExample {
    ...
    // PARSE_CONFIGURATION: Evaluated while parsing @Configuration classes (before bean definitions
    // exist)
    // REGISTER_BEAN: Evaluated when registering bean definitions (allows inspecting other bean
    // definitions)
    public static class OnDatabaseAvailableCondition implements ConfigurationCondition {
        @Override
        public ConfigurationPhase getConfigurationPhase() {
            // Evaluated during bean definition registration phase so we can inspect existing bean
            // definitions
    ...
    public static void main(String[] args) {
        ...
        ConfigurationCondition.ConfigurationPhase phase =
                condition.getConfigurationPhase(); // REGISTER_BEAN
        boolean isRegisterPhase =
                (phase == ConfigurationCondition.ConfigurationPhase.REGISTER_BEAN); // true
    }
}
